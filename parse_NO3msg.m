...
%
% INPUTS:
%	isus_file  = Path to float message file name as a string
%
% OUTPUTS:
...
%     	spec.CalibrationFile = calibration file  name
%
%
% EXAMPLE:
...
%                   subsitute EOP date
% 12/20/22 - JP - added code to recover calibration file name if it exists
% 05/22/23 - ECR (Sea-Bird) - added code to store calibration file name, message file
%            name, Current Time (UTC), FW nitrate value, FW AbsorbanceFitResiduals
...
% isus_file = 'C:\temp\12778.005.isus';
% isus_file ='C:\temp\18110.006.isus';
%isus_file = 'C:\temp\12709.090.isus';
%isus_file = 'C:\temp\8486.009.isus';

%isus_file ='C:\temp\18013.058.isus';
%isus_file ='C:\temp\18013.010.isus';
%isus_file ='C:\temp\20043.004.isus';
%isus_file ='C:\temp\12745.154.isus';
...
while ischar(tline)
    ...
    % get end profile time in case dura file has bad time stamp, i.e. 18110
...
if data_ct == 0
    ...
    if rem(spec_length,1) ~= 0 % each hex value described by 4 char
    ...
    spec.CAN_T      = spec.SDN;
    spec.NO3fw      = spec.SDN;
    spec.AbsorbanceFitResiduals = spec.SDN;
...
SN_filt = 'Datalogger\s+Serial\s+Num|SUNA\s+Serial\s+Number'; % SUNA/APEX SN line

while ischar(tline)
    % GET HEADER LINE INFO
    if  hdr_chk == 0 && ~isempty(regexp(tline,'^H','once'))
        hdr_ct = hdr_ct +1;
        if regexp(tline,'Current\s+Time\s+\(UTC\)','once') % Current Time (UTC)
            % use to match to cal file calibration date
            tmp = regexp(tline,',','split');
            spec.CurrentTimeUTC = strip(tmp{3});
        end        
        ...
        if regexp(tline,'Calibration\s+File','once') % calibration filename
            % use to match calibration file
            tmp = regexp(tline,',','split');
            spec.CalibrationFile = strip(tmp{3});
        end
        ...
        if isempty(spec.SN) && ~isempty(regexp(tline,SN_filt,'once')) % GET ISUS/SUNA SN
            tmp = regexp(tline,',','split');
            spec.SN = strip(tmp{3});
        end
    end
    ...
    if  ~isempty(regexp(tline,'^0x','once')) % data line
        ...
        if ~isempty(dline{1,25})
            ...
            if length(dline{1,25}{1}) == hex_length %&& dline{1,7} ~= -1
                ...
                spec.CAN_T(line_ct)  = dline{1,12};
                spec.NO3fw(line_ct)  = dline{1,21}; % ECR: firmware-estimated nitrate
                spec.AbsorbanceFitResiduals(line_ct) = dline{1,22}; % ECR [RMS]
